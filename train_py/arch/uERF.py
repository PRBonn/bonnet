  ...
  def build_graph(self, img_pl, train_stage, data_format="NCHW"):
    ...
    with tf.variable_scope("encoder"):
      ...
      with tf.variable_scope("Block2"):
        print("Block 2")
        block2_lyr1 = lyr.uERF_downsample(block1_lyr1, n_k_lyr[1], 3,
      ...
      with tf.variable_scope("Block3"):
        print("Block 3")
        block3_lyr1 = lyr.uERF_downsample(block2_lyr5, n_k_lyr[2], 3,
      ...
      with tf.variable_scope("Block4"):
        ...
        with tf.variable_scope("unpool"):
          block4_unpool = lyr.upsample_layer(code,
                                             ...
                                             data_format=data_format)
          if data_format == "NCHW":
            block4_bypass = tf.concat(
                [block4_unpool, block_2], 1)
          else:
            block4_bypass = tf.concat(
                [block4_unpool, block_2], 3)
          block4_reduced = lyr.conv_layer(block4_bypass, n_k_lyr[3], [1, 1], 1,
                                          train_stage and train_lyr[15],
                                          summary, bnorm=False, relu=True,
                                          data_format=data_format)

        with tf.variable_scope("non-bt-1"):
          block4_lyr1 = lyr.uERF_non_bt(block4_reduced, 3,
      ...
      with tf.variable_scope("Block5"):
        ...
        with tf.variable_scope("unpool"):
          block5_unpool = lyr.upsample_layer(block_4,
                                             ...
                                             data_format=data_format)
          if data_format == "NCHW":
            block5_bypass = tf.concat(
                [block5_unpool, block_1], 1)
          else:
            block5_bypass = tf.concat(
                [block5_unpool, block_1], 3)
          block5_reduced = lyr.conv_layer(block5_bypass, n_k_lyr[4], [1, 1], 1,
                                          train_stage and train_lyr[18],
                                          summary, bnorm=False, relu=True,
                                          data_format=data_format)

        with tf.variable_scope("non-bt-1"):
          block5_lyr1 = lyr.uERF_non_bt(block5_reduced, 3,
      ...
      with tf.variable_scope("Block6"):
        ...
        with tf.variable_scope("unpool"):
          block6_unpool = lyr.upsample_layer(block_5,
                                             ...
                                             data_format=data_format)
          if data_format == "NCHW":
            block6_bypass = tf.concat([block6_unpool, n_img], 1)
          else:
            block6_bypass = tf.concat([block6_unpool, n_img], 3)

        with tf.variable_scope("non-bt-1"):
          block6_lyr1 = lyr.uERF_non_bt(block6_bypass, 3,
